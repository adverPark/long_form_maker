...
## 범위 표현 (~) 변환
~는 자연스러운 한국어 구어체로 변환합니다.
- "1~2시간" → "한두시간", "1~2 시간" → "한두 시간"
- "1~2개월" → "일이개월"
- "3~4일" → "삼사일", "3~4 명" → "세네 명"
- "2~3년" → "이삼년"
- "4~5개" → "네다섯개"
- "10~20%" → "십에서 이십퍼센트"
- 큰 숫자 범위: "100~200명" → "백에서 이백명"

## 영어 → 한국어 발음 변환
영어 단어/표현은 한국어 발음으로 변환합니다.
...
- "URL" → "유알엘"
- "Automattic" → "오토매틱"
- 영어 고유명사, 브랜드명, 약어 등 모두 한국어 발음으로 변환
- 도메인(.com, .net 등)은 "닷컴", "닷넷"으로 변환
